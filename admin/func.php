...
function make_rooms_form() {
	...
	$rc = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("myxoopschat_rooms")."");
...
function make_editroom_form($room) {
	...
	$rc = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("myxoopschat_rooms")." WHERE room_name = '".$room."'");
...
function save_room($id,$name,$type,$group,$oldroom) {
	...
	$rc = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("myxoopschat_rooms")." WHERE rid = ".$id."");
	...
	if ( $room[2] == 1 ) {
		$qc = $xoopsDB->query("UPDATE ".$xoopsDB->prefix("myxoopschat_rooms")." SET room_name = '$name' WHERE rid = '$id'");
		//   $qc = "UPDATE ".$xoopsDB->prefix("myxoopschat_rooms")." SET room_name = '$name' WHERE rid = '$id'";
	}

	if ( $room[2] == 2 ) {
		$qc = $xoopsDB->query("UPDATE ".$xoopsDB->prefix("myxoopschat_rooms")." SET room_name = '$name', room_type = $type WHERE rid = '$id'");
		if ( $type == "1" ) {
	...
	$qc = $xoopsDB->query("UPDATE ".$xoopsDB->prefix("myxoopschat_messages")." SET chatroom = '$name' WHERE chatroom = '$oldroom'");
	$err = $xoopsDB->error();
	if ( $err == 0 ) {
...
function print_exoops_groups($rid=-1) {
	...
	$rc = $xoopsDB->query("SELECT g.groupid, g.name, v.rid FROM ".$xoopsDB->prefix("groups")." as g LEFT OUTER JOIN ".$xoopsDB->prefix("myxoopschat_visibility")." as v ON g.groupid = v.groupid and v.rid = $rid");
...
function update_room_groups($rid,$rmgroup=array()) {
	...
	$qc = $xoopsDB->query("DELETE FROM ".$xoopsDB->prefix("myxoopschat_visibility")." WHERE rid = $rid");
	...
	if (count($rmgroup) > 0) {
		...
		$err = $xoopsDB->query("INSERT INTO ".$xoopsDB->prefix("myxoopschat_visibility")." VALUES $valueslist");
...
function delete_room($rname,$rcancel) {
	...
	$rc = $xoopsDB->query("SELECT room_type, rid FROM ".$xoopsDB->prefix("myxoopschat_rooms")." WHERE room_name = '$rname'");
	...
	if ( $rcancel == 0 ) {
		...
		$qc = $xoopsDB->queryF("DELETE FROM ".$xoopsDB->prefix("myxoopschat_rooms")." WHERE room_name = '$rname'");
		$qc = $xoopsDB->queryF("DELETE FROM ".$xoopsDB->prefix("myxoopschat_messages")." WHERE chatroom = '$rname'");
		$err = $xoopsDB->error();
		if ( $err == 0 ) {
...
function create_room($rmname,$rmtype,$rmgroup) {
	...
	$proceed = true;
	if ( $rmtype == "1" ) {
		$rc = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("myxoopschat_rooms")." WHERE room_type = 1");
		if ( $xoopsDB->getRowsNum($rc) == 0 ) {
			$proceed = true;
		} else {
			$proceed = false;
	...
	$rc = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("myxoopschat_rooms")." WHERE room_name = '$rmname'");
	if ( $xoopsDB->getRowsNum($rc) != 0 ) {
		$proceed = false;
	}
	...
	if ( $proceed == true ) {
		if (empty($rmgroup) || is_array($rmgroup)) {
			$qc = $xoopsDB->query("INSERT INTO ".$xoopsDB->prefix("myxoopschat_rooms")." VALUES ('', '$rmname', '$rmtype')");
			$err = $xoopsDB->error();
			if ( $err != 0 ) {
				echo _AM_XCHAT_RDBERR."<br><br><b>.$err.</b>";
			} elseif ($rmtype == "3") {
